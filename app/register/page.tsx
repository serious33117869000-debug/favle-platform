...
import { MessageCircle, Mail, Lock, User, Gamepad2 } from 'lucide-react';
    ...
    <div className={`min-h-screen bg-[var(--background)] flex items-center justify-center px-4 py-20`} dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className={`text-3xl font-bold text-[var(--foreground)] mb-2`}>{t('auth.startJourney')}</h1>
          <p className="text-[var(--text-light)]">{t('auth.joinCommunity')}</p>
        </div>
        ...
        <div className={`${theme === 'dark' ? 'bg-white/10' : 'bg-white'} backdrop-blur-sm rounded-2xl p-8 border border-[var(--card-border)] shadow-lg`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6">
            <div>
              <label className={`block text-[var(--text-light)] mb-2 text-sm`}>{t('auth.name')}</label>
              <div className="relative">
                <User className={`absolute ${isRTL ? 'right' : 'left'}-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-[var(--text-muted)]`} />
                <input
                  type="text"
                  value={formData.name}
            ...
            <div>
              <label className={`block text-[var(--text-light)] mb-2 text-sm`}>{t('auth.confirmPassword')}</label>
              <div className="relative">
                <Lock className={`absolute ${isRTL ? 'right' : 'left'}-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-[var(--text-muted)]`} />
                <input
                  type="password"
